...
def validate_html(html:str)->None:
    ...
    required.extend(f'<option value="{x}">' for x in LANGUAGES)
    required.extend(f'hreflang="{x}" href="https://listiaapp.com/?lang={x}"' for x in LANGUAGES)
...
def build_worker(html:str,cloudco_html:str,footer_js:str,logo_b64:str)->str:
    validate_html(html);validate_cloudco(cloudco_html,footer_js)
...
def main()->int:
    ...
    repo_commercial=Path("commercial")
    d=repo_commercial if (repo_commercial/"cloudco.html").exists() else a.input.parent
    worker=build_worker(
        html,
        (d/"cloudco.html").read_text(encoding="utf-8"),
        (d/"footer-legal.js").read_text(encoding="utf-8"),
        b64(d/"cloudco-assets"/"cloudco-logo-official.webp"),
    )
