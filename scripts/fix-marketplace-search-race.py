...
already_fixed = new in s and new_load in s
if not already_fixed:
    if old not in s or old_load not in s:
        raise SystemExit('Marketplace search state is neither legacy nor recognized fixed state')
    s=s.replace(old,new,1).replace(old_load,new_load,1)
    market.write_text(s,encoding='utf-8')

for path in (config,sw):
    text=path.read_text(encoding='utf-8')
    if '/marketplace.js?v=12' in text:
        text=text.replace('/marketplace.js?v=12','/marketplace.js?v=13')
        if path==sw and 'visible-marketplace-v1-search-v13"' not in text:
            text=text.replace('visible-marketplace-v1"','visible-marketplace-v1-search-v13"',1)
        path.write_text(text,encoding='utf-8')
    elif '/marketplace.js?v=13' not in text:
        raise SystemExit(f'{path.name}: unexpected Marketplace runtime version')

print('Marketplace latest-search-wins contract is present; runtime v13 is consistent')
