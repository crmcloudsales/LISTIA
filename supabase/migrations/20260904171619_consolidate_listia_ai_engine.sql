...
insert into private.ai_route_policies(task_type,quality_tier,strategy,primary_models,reviewer_models,fallback_models,required_validators,max_parallel,max_attempts,escalate_on_failure,optimization_objective,active,notes) values
...
on conflict(task_type,quality_tier) do update set strategy=excluded.strategy,primary_models=excluded.primary_models,reviewer_models=excluded.reviewer_models,fallback_models=excluded.fallback_models,required_validators=excluded.required_validators,max_parallel=excluded.max_parallel,max_attempts=excluded.max_attempts,escalate_on_failure=excluded.escalate_on_failure,optimization_objective=excluded.optimization_objective,active=true,notes=excluded.notes;
...
do $$ begin if not exists(select 1 from vault.secrets where name='listia_ai_engine_secret') then perform vault.create_secret(replace(gen_random_uuid()::text,'-','')||replace(gen_random_uuid()::text,'-',''),'listia_ai_engine_secret','LISTIA internal AI Engine service authentication'); end if; end $$;
