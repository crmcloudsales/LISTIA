...
as $function$
  with filtered as (
    select
      ...
      ml.cover_image_url, ml.gallery, ml.locale, ml.published_at, ml.created_at
  ...
  select
    ...
    ml.longitude,
    ml.cover_image_url,
    coalesce(ml.gallery,'[]'::jsonb) as gallery,
    ml.locale,
    ml.published_at,
...
revoke all on function public.marketplace_public_feed_v3(integer,integer,text,text,text,numeric,numeric,numeric) from public, authenticated;
grant execute on function public.marketplace_public_feed_v3(integer,integer,text,text,text,numeric,numeric,numeric) to anon;
