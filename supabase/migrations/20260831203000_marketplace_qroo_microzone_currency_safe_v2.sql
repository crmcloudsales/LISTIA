drop view if exists public.marketplace_qroo_microzones;
create view public.marketplace_qroo_microzones as
with clean as (
  select l.*,
...
from clean
group by canonical_place,municipality,coalesce(extracted_colonia, canonical_place),extracted_postal_code,price_currency
having count(*) filter(where not obvious_out_of_state)>0;

revoke all on public.marketplace_qroo_microzones from anon,authenticated;
grant select on public.marketplace_qroo_microzones to service_role;
