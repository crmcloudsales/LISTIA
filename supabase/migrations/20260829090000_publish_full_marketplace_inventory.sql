-- Canonical marketplace publication policy: every listing already marked
-- visibility=public and status=published is eligible for the public Marketplace.
-- Keep the base table closed to browser roles; expose curated fields through RPCs.

drop policy if exists marketplace_listings_public_safe_select on public.marketplace_listings;
drop policy if exists marketplace_listings_public_select on public.marketplace_listings;

create policy marketplace_listings_public_select
on public.marketplace_listings
for select
to anon, authenticated
using (visibility = 'public'::text and status = 'published'::text);

alter function public.marketplace_public_feed(integer) security definer;
alter function public.marketplace_public_feed_v2(integer, integer, text, text, text, numeric, numeric, numeric) security definer;
...
as $function$
  with filtered as (
    select
      ...
      ml.cover_image_url,
      case when jsonb_typeof(ml.gallery)='array' then ml.gallery else '[]'::jsonb end as gallery,
  ...
  select
        ...
        regexp_replace(
          regexp_replace(
            regexp_replace(
              regexp_replace(
                regexp_replace(
                  regexp_replace(coalesce(ml.description,''),'https?://[^[:space:]<>"'']+','','gi'),
                  'www\.[^[:space:]<>"'']+','','gi'
                ),
    ...
    ml.operation_type, ml.property_type, ml.price, ml.currency, ml.location_text, ml.city,
    ml.state_region, ml.country_code, ml.bedrooms, ml.bathrooms, ml.parking_spaces, ml.area_m2,
    ml.latitude, ml.longitude, ml.cover_image_url, coalesce(ml.gallery,'[]'::jsonb), ml.locale,
    ml.published_at, count(*) over() as total_count
  from filtered ml
...
grant execute on function public.marketplace_public_feed(integer) to anon, authenticated;
...
grant execute on function public.marketplace_public_feed_v3(integer, integer, text, text, text, numeric, numeric, numeric) to anon, authenticated;
