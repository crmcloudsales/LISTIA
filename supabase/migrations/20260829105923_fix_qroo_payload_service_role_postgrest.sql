-- PostgREST already enforces the function EXECUTE grant using the service-role JWT.
-- Do not duplicate that authorization with brittle current_user/session_user string checks.
-- Client roles remain explicitly revoked below.
...
begin
  if jsonb_typeof(p_payload) <> 'array' then
    raise exception 'qroo_payload_not_array';
  end if;

  payload_count := jsonb_array_length(p_payload);
  if payload_count < 1 or payload_count > 250 then
    raise exception 'qroo_payload_size_invalid';
  end if;
  ...
  if src is null then
    raise exception 'qroo_source_missing';
  end if;
  ...
  with raw as (
    ...
    select p.*,
      ...
      case
        when private.marketplace_image_url_is_valid(p.raw_cover) then p.raw_cover
        else (
          select g.url
          from jsonb_array_elements_text(p.raw_gallery) with ordinality as g(url,ord)
          where private.marketplace_image_url_is_valid(g.url)
    ...
    select
      src,slug,title,description,operation_type,property_type,price,currency,
      location_text,city,state_region,country_code,bedrooms,bathrooms,area_m2,
      cover_image_url,gallery,
      jsonb_build_object(
        'internal_external_id',external_id,
        'internal_municipality',x->>'municipality',
        'internal_ingestion','qroo_private_oidc_v1'
      ),
