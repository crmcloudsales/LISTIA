...
create or replace function public.marketplace_public_feed_edge(
...
returns table(
  ...
  longitude numeric, cover_image_url text, gallery jsonb, locale text, published_at timestamptz,
  total_count bigint
)
language sql
stable
...
with filtered as (
  select ml.id,ml.slug,ml.title,ml.description,ml.operation_type,ml.property_type,ml.price,ml.currency,
         ...
         ml.parking_spaces,ml.area_m2,ml.latitude,ml.longitude,
  ...
  where ml.visibility='public' and ml.status='published'
    ...
    and (
      ...
      or position(lower(left(btrim(p_q),120)) in lower(concat_ws(' ',ml.title,ml.location_text,ml.city,ml.state_region,ml.country_code,ml.property_type))) > 0
    )
)
select ml.id,ml.slug,ml.title,
       ...
       ml.cover_image_url,ml.gallery,ml.locale,ml.published_at,count(*) over() as total_count
from filtered ml
...
revoke all on function public.marketplace_public_feed_edge(integer,integer,text,text,text,numeric,numeric,numeric) from public, anon, authenticated;
grant execute on function public.marketplace_public_feed_edge(integer,integer,text,text,text,numeric,numeric,numeric) to service_role;

revoke execute on function public.marketplace_public_feed(integer) from public, anon, authenticated;
revoke execute on function public.marketplace_public_feed_v2(integer,integer,text,text,text,numeric,numeric,numeric) from public, anon, authenticated;
revoke execute on function public.marketplace_public_feed_v3(integer,integer,text,text,text,numeric,numeric,numeric) from public, anon, authenticated;

grant execute on function public.marketplace_public_feed(integer) to service_role;
grant execute on function public.marketplace_public_feed_v2(integer,integer,text,text,text,numeric,numeric,numeric) to service_role;
grant execute on function public.marketplace_public_feed_v3(integer,integer,text,text,text,numeric,numeric,numeric) to service_role;
