-- LISTIA absolute media rule: a generic site placeholder does not count as a property image.
-- This migration protects Marketplace writes and the Q.Roo state-wide ingestion path.
...
revoke execute on function private.marketplace_image_url_is_valid(text) from public, anon, authenticated;
grant execute on function private.marketplace_image_url_is_valid(text) to service_role;
...
begin
  ...
  if not private.marketplace_image_url_is_valid(new.cover_image_url)
     and coalesce(jsonb_array_length(new.gallery),0)=0 then
    raise exception using
      errcode='23514',
      message='valid_image_required',
      detail='LISTIA marketplace listings require at least one valid property image; generic placeholders do not qualify.';
  end if;
...
revoke execute on function private.enforce_marketplace_listing_image_required() from public, anon, authenticated;
...
begin
  ...
  with raw as (select x from jsonb_array_elements(payload) x), parsed as (
    ...
    from parsed where external_id~'^[0-9]+$' and slug~'^qroo-[0-9]+$'
  ...
  return jsonb_build_object('payload',payload_count,'inserted',inserted_count,'skipped',skipped_count,'image_required',true,'generic_placeholders_rejected',true,'chunk',p_chunk_url);
end;
$function$;

revoke execute on function public.ingest_qroo_crawl_chunk(text) from public, anon, authenticated;
grant execute on function public.ingest_qroo_crawl_chunk(text) to service_role;
