...
-- This file persists an already-applied production migration; it is not a new behavior change.
...
revoke all on table private.marketplace_interest_rate_limits from public, anon, authenticated;
...
begin
  if p_user_id is null
     or p_max_requests < 1 or p_max_requests > 50
...
revoke execute on function public.consume_marketplace_interest_rate_limit(uuid,integer,integer)
  from public, anon, authenticated;
grant execute on function public.consume_marketplace_interest_rate_limit(uuid,integer,integer)
  to service_role;
