...
async function consumeRateLimit(principalId: string, organizationId: string) {
  const action = 'property_processing_start'
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!propertyId) return json(req, { error: 'property_id_required' }, 400)
    ...
    const [billing] = await sql`
      select plan_key, billing_status, access_state, usage_markup_percent
      from public.organization_billing
      where organization_id=${organizationId}::uuid
      limit 1
    `
    const effectivePlan = String(billing?.plan_key || 'free')
    const accessState = String(billing?.access_state || 'active')
    if (accessState === 'payment_blocked') return json(req, { error: 'payment_blocked' }, 402)

    const [property] = await sql`
      select id,organization_id,title,operation_type,property_type,description,price,currency,
             commission_text,location_text,postal_code,status,source,locale,processing_state,created_at
    ...
    const assets = await sql`
      select id,asset_type,original_name,mime_type,size_bytes,storage_bucket,storage_path,metadata,created_at
      from public.property_assets
      where property_id=${propertyId}::uuid and organization_id=${organizationId}::uuid
    ...
    const submitted = {
      ...
      commission_text: property.commission_text || null,
      location_text: property.location_text || null,
      postal_code: property.postal_code || null,
    }

    const missing: string[] = []
    if (!property.operation_type) missing.push('operation_type')
    if (!property.property_type) missing.push('property_type')
    if (!property.location_text) missing.push('location_text')
    if (!property.description) missing.push('description')
    if (property.price === null || property.price === undefined) missing.push('price')
    if (!property.currency) missing.push('currency')

    const meaningfulDirectInput = Boolean(property.description || property.location_text || property.price !== null)
    const assetCount = assets.length
    const hasMaterial = meaningfulDirectInput || assetCount > 0
    const nextStage = hasMaterial ? 'ready_for_ai' : 'needs_input'
    ...
    await sql.begin(async (tx) => {
      await tx`
        insert into public.property_processing_state(
          ...
          ${propertyId}::uuid,${organizationId}::uuid,${nextStage},${assetCount},${JSON.stringify(manifest)}::jsonb,
          ${JSON.stringify(submitted)}::jsonb,${missing}::text[],now(),now(),null,now()
        )
      ...
      await tx`
        ...
        set status=${hasMaterial ? 'processing' : 'material_received'},
            processing_state=coalesce(processing_state,'{}'::jsonb) || ${JSON.stringify({
              stage: nextStage,
              asset_count: assetCount,
              missing_fields: missing,
              prepared_at: now,
              next_action: hasMaterial ? 'ai_extraction_pending' : 'request_more_material',
            })}::jsonb,
            updated_at=now()
        where id=${propertyId}::uuid and organization_id=${organizationId}::uuid
      `
    ...
    console.error('property-processing-start', error)
