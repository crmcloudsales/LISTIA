...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    await sql.begin(async tx=>{
      ...
      await tx`update public.properties set status=${propertyStatus},processing_state=coalesce(processing_state,'{}'::jsonb)||${JSON.stringify({stage:nextStage,asset_count:assetCount,missing_fields:missing,prepared_at:now,next_action:nextAction,processing_mode:'deterministic_fallback_plus_ai_queue',ai_status:hasMaterial?'queueing':'not_applicable'})}::jsonb,updated_at=now() where id=${propertyId}::uuid and organization_id=${organizationId}::uuid`
    })
    let aiJobId:string|null=null,aiStatus=hasMaterial?'queue_failed':'not_applicable'
    if(hasMaterial){try{const[queued]=await sql`select private.enqueue_property_ai_job(${propertyId}::uuid,${organizationId}::uuid,${JSON.stringify(manifest)}::jsonb) as job_id`;aiJobId=queued?.job_id?String(queued.job_id):null;aiStatus=aiJobId?'queued':'queue_failed';await sql`update public.properties set processing_state=coalesce(processing_state,'{}'::jsonb)||${JSON.stringify({ai_status:aiStatus,ai_job_id:aiJobId})}::jsonb,updated_at=now() where id=${propertyId}::uuid and organization_id=${organizationId}::uuid`}catch(error){console.error('property-processing-start queue',String((error as Error)?.message||error).slice(0,240))}}
    return json(req,{ok:true,property_id:propertyId,property_status:propertyStatus,stage:nextStage,asset_count:assetCount,missing_fields:missing,effective_plan:effectivePlan,next_action:nextAction,processing_mode:'deterministic_fallback_plus_ai_queue',ai_status:aiStatus,ai_job_id:aiJobId})
