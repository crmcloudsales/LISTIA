...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: member } = await admin.from('organization_members')
      .select('role,status')
      .eq('organization_id', String(property.organization_id))
      .eq('user_id', user.id)
      .eq('status', 'active')
      .maybeSingle()
    if (!member || !['owner','admin'].includes(String(member.role))) {
      return json(req, { error: 'property_admin_required' }, 403)
    }

    const result = await sql.begin(async (tx) => {
      const [draft] = await tx`
        select property_id, organization_id, status, missing_fields, version
      ...
      if (Array.isArray(draft.missing_fields) && draft.missing_fields.length > 0) {
        return { error: 'draft_incomplete', status: 409, missing_fields: draft.missing_fields }
      }
      ...
      await tx`
        ...
        set stage='ready', missing_fields='{}'::text[], updated_at=now(), error_message=null
        where property_id=${propertyId}::uuid
      `
      await tx`
        ...
        set status='ready',
            processing_state=coalesce(processing_state,'{}'::jsonb) || jsonb_build_object(
              ...
              'approved_by',${user.id}::text,
              'approved_at',now(),
              'next_action','publish_or_distribute'
            ),
            updated_at=now()
        where id=${propertyId}::uuid
      `
    ...
    console.error('property-draft-approve', error)
