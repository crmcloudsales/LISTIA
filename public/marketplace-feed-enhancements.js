(() => {
  ...
  function safeMarketplaceUrl(raw){if(!raw.includes(LEGACY_MARK))return raw;const i=raw.indexOf('/rest/v1/');if(i<0)return raw;return`${raw.slice(0,i)}${SAFE_MARK}?p_limit=1000`}
  ...
  window.fetch=async function(input,init){const raw=typeof input==='string'?input:(input?.url||''),safeUrl=safeMarketplaceUrl(raw);let target=input;if(safeUrl!==raw)target=typeof input==='string'?safeUrl:new Request(safeUrl,input);const response=await nativeFetch(target,init);const finalUrl=typeof target==='string'?target:(target?.url||raw);if((finalUrl.includes(SAFE_MARK)||finalUrl.includes(SAFE_V2_MARK))&&response.ok){response.clone().json().then(mergeRows).catch(()=>{})}return response};
  ...
  function enhanceDetail(row){const screen=document.getElementById('screen-marketplace-detail');if(!screen||!screen.classList.contains('active')||!row)return;const media=screen.querySelector('.marketplace-detail-media');const main=media?.querySelector('img');const gallery=normalizeGallery(row);if(media&&main&&gallery.length){main.src=gallery[0];let rail=media.querySelector('.listia-marketplace-gallery');if(!rail){rail=document.createElement('div');rail.className='listia-marketplace-gallery';media.append(rail)}rail.innerHTML='';gallery.forEach((url,i)=>{const b=document.createElement('button');b.type='button';b.className=`listia-marketplace-thumb${i===0?' active':''}`;const img=document.createElement('img');img.src=url;img.alt='';img.loading='lazy';b.append(img);b.onclick=()=>{main.src=url;rail.querySelectorAll('.listia-marketplace-thumb').forEach(x=>x.classList.remove('active'));b.classList.add('active')};rail.append(b)})}
    if(Number.isFinite(Number(row.latitude))&&Number.isFinite(Number(row.longitude))&&!screen.querySelector('.listia-marketplace-map')){const lat=Number(row.latitude),lon=Number(row.longitude),d=.006;const box=document.createElement('section');box.className='listia-marketplace-map';const h=document.createElement('strong');h.textContent=locale().toLowerCase().startsWith('es')?'Ubicación':'Location';const frame=document.createElement('iframe');frame.loading='lazy';frame.referrerPolicy='no-referrer';frame.title=h.textContent;frame.src=`https://www.openstreetmap.org/export/embed.html?bbox=${encodeURIComponent(lon-d)},${encodeURIComponent(lat-d)},${encodeURIComponent(lon+d)},${encodeURIComponent(lat+d)}&layer=mapnik&marker=${encodeURIComponent(lat)},${encodeURIComponent(lon)}`;box.append(h,frame);const form=screen.querySelector('.marketplace-interest-grid');if(form)form.before(box);else screen.querySelector('.marketplace-detail-body')?.append(box)}
  }

  function enhanceCards(){document.querySelectorAll('#marketplaceGrid .marketplace-card').forEach(card=>{const actions=card.querySelector('.marketplace-actions');if(!actions)return;actions.querySelectorAll('.secondary').forEach(b=>b.remove());const interest=actions.querySelector('.primary')||actions.querySelector('button');if(!interest)return;actions.classList.add('listia-interest-only');if(interest.dataset.listiaInterest==='1')return;interest.dataset.listiaInterest='1';interest.addEventListener('click',()=>{const row=rowFor(card);if(!row)return;window.LISTIA_MARKETPLACE_SELECTED=row;captureInterest(row);setTimeout(()=>enhanceDetail(row),40);setTimeout(()=>enhanceDetail(row),180)},true)})}
