...
- Canonical media/distribution companion map, including Zernio, Runway Aleph 2.0, Seedance, Gemini Omni, Veo, Pika, Higgsfield and voice/media supporting tools.
- Private AI provider/model registry.
- Private benchmark-case registry.
- Private AI run telemetry/model score tables.
- AI route policies for property extraction, flyer copy/render, protected advisor/property fidelity, video generation, dedicated video editing and quality review.
- Private provider runtime registry that separates provider/model knowledge from actual adapter/credential readiness.
- `ai-route-plan` Edge Function v4: authenticated route planning, organization/property authorization, policy candidates, runtime credentials/health gating, dedicated `video_edit` routing, validators/fallbacks/escalation metadata.
- Pika API registered as a candidate media gateway with its runtime disabled until credentials and health checks are configured.
- Runway Aleph registry normalized to current `Aleph 2.0` / API model id `aleph2`.

## VERIFIED
- LISTIA Supabase project: `zvzafiarwerbuoaccnoz` is the live backend used by this workstream.
- AI engine registry currently contains 17 providers, 27 model entries, 8 benchmark cases and 19 route policies.
- Dedicated `video_edit` has Q2, Q3 and Q4 policies with Runway Aleph 2.0 as primary candidate and Seedance 2.5 as fallback.
...
- Supabase and GitHub are synchronized through migration 37 (`20260824065145_add_video_edit_route_and_pika_gateway`).
- `ai-route-plan` v4 is ACTIVE with JWT verification enabled.
...
- Pika API credential/benchmark before deciding whether it beats direct provider pricing for any media route.
- Zernio DistributionAdapter and `ZERNIO_API_KEY`; Zernio remains outside the generative AI Router because it is a distribution/social/messaging layer.
...
3. Add Runway Aleph 2.0 adapter behind `video_edit`, then benchmark it specifically on protected-property/advisor footage.
4. Add Zernio as DistributionAdapter after the content Final Quality Gate, not inside media generation.
5. Benchmark Pika API Club against direct provider calls for identical media tasks to measure accepted-output cost, latency and feature completeness.
6. Record every provider call in `private.ai_runs` and every billable provider cost through Gestiones.
7. Mark a runtime configured only after its server-side secret exists; mark it healthy only after a real API health/test call succeeds; only then allow the Router to return `runtime_ready=true`.
8. Only mark models benchmarked/active after reproducible quality tests pass.
